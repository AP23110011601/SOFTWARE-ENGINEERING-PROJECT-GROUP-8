...
const DashboardComplete = () => {
  ...
  const [irrigationZones, setIrrigationZones] = useState({
    zone1: { active: false, moisture: 45, temperature: 28, autoMode: true },
    zone2: { active: false, moisture: 52, temperature: 26, autoMode: true },
    zone3: { active: false, moisture: 38, temperature: 30, autoMode: false }
  });
  ...
  const generateMockChartData = () => {
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      moistureData.push(40 + Math.random() * 30);
    }
  ...
  const updateSensorData = () => {
    setIrrigationZones(prev => ({
      zone1: { 
        ...prev.zone1, 
        moisture: Math.max(20, Math.min(80, prev.zone1.moisture + (Math.random() - 0.5) * 5)),
        temperature: Math.max(15, Math.min(40, prev.zone1.temperature + (Math.random() - 0.5) * 2))
      },
      zone2: { 
        ...prev.zone2, 
        moisture: Math.max(20, Math.min(80, prev.zone2.moisture + (Math.random() - 0.5) * 5)),
        temperature: Math.max(15, Math.min(40, prev.zone2.temperature + (Math.random() - 0.5) * 2))
      },
      zone3: { 
        ...prev.zone3, 
        moisture: Math.max(20, Math.min(80, prev.zone3.moisture + (Math.random() - 0.5) * 5)),
        temperature: Math.max(15, Math.min(40, prev.zone3.temperature + (Math.random() - 0.5) * 2))
      }
    }));
  };

  const handleIrrigationToggle = (zone) => {
    setIrrigationZones(prev => ({
      ...prev,
      [zone]: { ...prev[zone], active: !prev[zone].active }
    }));
    
    const zoneData = irrigationZones[zone];
    if (!zoneData.active) {
      notificationTypes.irrigation(`Irrigation started in ${zone}`, zone.split('zone')[1]);
    } else {
      notificationTypes.irrigation(`Irrigation stopped in ${zone}`, zone.split('zone')[1]);
    }
  };

  const handleAutoModeToggle = (zone) => {
    setIrrigationZones(prev => ({
      ...prev,
      [zone]: { ...prev[zone], autoMode: !prev[zone].autoMode }
    }));
    
    notificationTypes.info(`Auto mode ${irrigationZones[zone].autoMode ? 'disabled' : 'enabled'} for ${zone}`);
  };
  ...
  const mockSensorData = {
    ...
    ph: 6.8,
    rainfall: 2.5,
    zone1: irrigationZones.zone1,
    zone2: irrigationZones.zone2,
    zone3: irrigationZones.zone3
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Complete farm monitoring and control system
              </p>
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="p-3 bg-orange-100 dark:bg-orange-900 rounded-lg hover:bg-orange-200 dark:hover:bg-orange-800 transition-colors"
              >
                <Cpu size={20} className="text-orange-600 dark:text-orange-400" />
              </button>
              <button
                onClick={() => navigate("/profile")}
                className="p-3 bg-indigo-100 dark:bg-indigo-900 rounded-lg hover:bg-indigo-200 dark:hover:bg-indigo-800 transition-colors"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${(currentData.temperature / 40) * 100}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-1">
                    ...
                    <span>{currentData.soilMoisture}%</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div 
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-green-100 dark:border-gray-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(irrigationZones).map(([zone, data]) => (
                <div key={zone} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={`font-medium ${getMoistureColor(data.moisture)}`}>
                        {data.moisture.toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400">Temperature:</span>
                      <span className={`font-medium ${getTemperatureColor(data.temperature)}`}>
                        {data.temperature.toFixed(1)}°C
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400">Mode:</span>
                      <span className={`font-medium ${data.autoMode ? 'text-blue-600' : 'text-orange-600'}`}>
                        {data.autoMode ? 'Auto' : 'Manual'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-2">
                    <button
                      onClick={() => handleIrrigationToggle(zone)}
                    ...
                    >
                      {data.active ? (
                        <><Power size={16} className="inline mr-1" /> Stop</>
                      ) : (
                    ...
                    <button
                      onClick={() => handleAutoModeToggle(zone)}
